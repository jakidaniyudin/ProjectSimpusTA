...
class LogicPemeriksaanIbuRepository  extends BaseLogicAbstract
{
    ...
    public function set($post, $data)
    {
        try {
            ...
            $id_subLayanan =  $this->CI->MasterSubLayanan_model->getByName('pemeriksaan_ibu');
    ...
    protected function BatchUpdateDataHelper($data)
    {
        ...
        foreach ($data as $index => $value) {
            if (isset($value['id']) && isset($value['value'])) {
                ...
                break;
                throw new ServiceException('not found for indexing', 500);
            }
        }
        return $batchData;
    }
