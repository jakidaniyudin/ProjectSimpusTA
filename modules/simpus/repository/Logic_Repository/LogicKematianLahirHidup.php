...
class LogicKematianLahirHidup  extends BaseLogicAbstract
{
    protected $CI;
    ...
    public function set($post, $data)
    {
        try {
            ...
            $id_subLayanan =  $this->CI->MasterSubLayanan_model->getByName('kematian_lahir_hidup');
            ...
            if ($id_record_detail != null) {
                ...
                throw new ServiceException('tidak ditemukan data yang id record Bayi', 500);
            }
        } catch (Exception $e) {
    ...
    protected function BatchUpdateDataHelper($data, $id_record_detail)
    {
        $batchData = [];
        $creatBatchData = [];
        $update_at = date('Y-m-d H:i:s');
    ...
    {
        $batchResult = $this->BatchUpdateDataHelper($data, $id_record_detail);
        if (empty($batchResult)) {
            throw new ServiceException('not found for collection id and value', 400);
        }
        if (!empty($batchResult['create'])) {
            $this->create($data, $id_record_detail);
        }
        return $this->CI->HasilPemeriksaan_model->update($batchResult['update']);
    }
