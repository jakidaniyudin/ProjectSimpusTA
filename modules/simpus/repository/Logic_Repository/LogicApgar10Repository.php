...
class LogicApgar10Repository  extends BaseLogicAbstract
{
    ...
    public function set($post, $data)
    {
        try {
            ...
            $id_subLayanan =  $this->CI->MasterSubLayanan_model->getByName('apgar10');
            ...
            if ($id_record_detail != null) {
                ...
                throw new ServiceException('tidak ditemukan data yang id record apgar 10', 500);
            }
        } catch (Exception $e) {
    ...
    protected function BatchUpdateDataHelper($data)
    {
        ...
        foreach ($data as $index => $value) {
            ...
            if (isset($value['id']) && isset($value['value'])) {
                $batchData[] = [
                    'id' => $value['id'],
                    'jawaban' => json_encode($jawaban, JSON_PRETTY_PRINT),
                    'update_at' => $update_at
                ];
            } else {
                break;
    ...
    public function update($data, $id_record_detail = null)
    {
        ...
        if (empty($batchResult)) {
            throw new ServiceException('not found for collection id and value apgar 10', 400);
        }
