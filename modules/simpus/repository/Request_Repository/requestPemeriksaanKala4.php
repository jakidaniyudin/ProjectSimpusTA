...
class requestPemeriksaanKala4 implements RequestInterface
{
    protected $CI;
    protected $validator;
    ...
    {
        return [
            'dataKala4[penatalaksanaanMasalah][value]' => 'required',
            'dataKala4[hasil][value]'                  => 'required',
            'dataKala4[masalahKalaIV][value]'           => 'required',
        ];
    }
    ...
    public function setDataRequest($request)
    {
        ...
        $request =  $request->post('dataKala4');
        $data =  [];
        if (!empty($data)) {
            return $data;
        } else {
