...
class  logicRepositoryPopUpRiwayatPribadi implements interfacePopUp
{
    ...
    {
        $data['n'] = $n;
        return  $this->CI->load->view('simpus/pop/v_riwayat_pribadi', $data);
    }
    ...
    public function list($propertis = null)
    {
        ...
        $data = array_map(function ($riwayat) {
            return [
                ...
                '<a class="btn btn-xs btn-info" onclick="pilihRiwayatPribadi(\'' . $riwayat->code . '\', \'' . $riwayat->value_set . '\',\'' . $riwayat->codesystem . '\',\'' . $riwayat->source_of_code . '\')">Pilih</a>' // kolom action
            ];
        }, $list);
