...
class objektifANCRepository implements InterfaceSubLayanan
{

    public function loadForm($load, $session, $encryption, $model)
    {
        $data =  $this->get_session($session, $encryption);
        $data['obsetri'] = $this->loadModel($model, $data['pasien_id']);
        $load->view('pelayanan/KIA/ANC/pelayanan_form/v_objectif', $data);
    }
    ...
    {
        $getObsetri =  $model->getById($pasien_id)->row();
        return $getObsetri;
    }
}
